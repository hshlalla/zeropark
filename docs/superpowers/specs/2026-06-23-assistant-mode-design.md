...
4. **에이전트별 도구 목록을 `App.params.tools`에 저장** — 관리자가 도구 선택

> **구현 결정 (spec 대비 단순화):** 도구 목록을 `App`에 새 DB 컬럼으로 두는 대신
> 기존 `App.params` JSON 안의 `tools` 키에 저장한다. `AppViewer`가 이미 `app.params`를
> 그대로 task params로 전달하므로([AppViewer.tsx](../../../packages/zeropark-web/src/pages/AppViewer.tsx)),
> DB 마이그레이션도 게이트웨이 주입 코드도 필요 없이 `task.params["tools"]`까지
> 자동으로 흐른다.
...
도구 목록은 `App.params` JSON 안의 `tools` 키에 저장한다 (예:
`{"model": "...", "tools": ["image","slides"]}`):

- **챗만 필요한 에이전트** → mode=`assistant`, `params.tools=[]` → 순수 대화.
- **도구 묶은 에이전트** → mode=`assistant`, `params.tools=["image","slides"]` 등.

**흐름:**

```
관리자가 에이전트 생성 (mode=assistant, tools 체크)
   ↓ App.params.tools 에 저장
사용자가 그 에이전트로 대화 → AppViewer가 app.params를 task.params로 전달
   ↓ (게이트웨이 추가 주입 불필요 — params가 이미 흐름)
AssistantEngine: task.params["tools"]로 도구 스펙을 필터링
...
| `packages/zeropark-web/src/pages/Dashboard.tsx` | 에이전트 생성 시 도구 체크박스 (mode=assistant일 때) → `params.tools` |
